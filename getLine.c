...
if ((*buf)[r - 1] == '\n')
{
(*buf)[r - 1] = '\0'; /*its remove trailing newline */
r--;
}
info->linecount_flag = 1;
remove_comments(*buf);
build_history_list(info, *buf, info->histcount++);
/* if (_strchr(*buf, ';')) is its a command chain? */
...
i = j + 1; /* its increment past nulled ';'' */
if (i >= len) /* its reached end of buffer? */
{
i = len = 0; /* the reset position and length */
info->cmd_buf_type = CMD_NORM;
}
*buf_p = p; /* its passes back pointer to current command position */
...
/**
* _getline -its gets the next line of input from STDIN
* @info:the parameter struct
